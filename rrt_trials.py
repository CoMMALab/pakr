...
def run_parameter_sweep():
    ...
    for env_name, config in ENV_CONFIGS.items():
        print(f"\n========== ENV: {env_name.upper()} (dt={config['dt']}, tte={config['tte']}) ==========")
        obstacles = helper.get_obs(f"envs/{env_name}.csv")
        
        # Inner Loop: Batch Sizes
        for b_size in BATCH_SIZES:
            k_factor = BRANCHING_FACTOR 
            ...
            # Warm-up (Important: Re-JIT because dt/tte might change kernel behavior)
            ...
            _ = jit_while(tree_init, sst_params, sim_params, CALLABLES_RESERVED, obstacles, 0, k_factor)
