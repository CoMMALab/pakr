...
# MAIN EXECUTION
# ------------------------------------------------------------------
def extract_solution(tree, goal_mask, start_idx):
    if jnp.sum(goal_mask) == 0:
        print("No solution found.")
        return None, None

    goal_local = jnp.argmax(goal_mask)
    goal_idx = start_idx + goal_local
    ...
    path = jnp.array(path[::-1])
    actions = jnp.array(actions[::-1])
...
if __name__ == "__main__":
    ...
    print("\nRunning single planning instance...")

    tree = rrtree.KinoTree.init(MAX_TREE_SIZE, sim_params.dims, sim_params.action_dims)
    tree, _ = rrtree.add_nodes(tree, init_state, jnp.zeros(1), -1, 0.0, 1)

    start_t = time.perf_counter()
    res = jit_while(tree, sst_params, sim_params, callables, obstacles, 0)
    tree, _, goal_mask, goal_count, states, start_idx, iter_num = jax.block_until_ready(res)
    duration = time.perf_counter() - start_t

    print(f"Planning finished in {duration:.3f}s | nodes {tree.tree_size} | iterations {iter_num}")

    path, actions = extract_solution(tree, goal_mask, start_idx)
    ...
    print("Replaying trajectory with full MJX rollout...")
    ...
    for action in actions[1:]:
        ...
        for _ in range(sst_params.time_to_evolve):
    ...
    np.save("videos/cartpole_traj.npy", traj)

    print(f"Saved trajectory with {len(traj)} states to videos/cartpole_traj.npy")