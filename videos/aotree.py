...
if __name__ == "__main__":
    ...
    # Initialize: { "Bucket_1": [], ... }
    solution_buckets = {key: [] for key in BUCKET_RANGES.keys()}
    TARGET_PER_BUCKET = 10
    

    def get_bucket_key(cost):
        for key, (min_c, max_c) in BUCKET_RANGES.items():
            if min_c <= cost < max_c:
                return key
        return None
    ...
    def buckets_full(buckets):
        for key, trajs in buckets.items():
            print(f"{key}: {len(trajs)} trajectories")
        return all(len(trajs) >= TARGET_PER_BUCKET for trajs in buckets.values())

    run_id = 0
    while not buckets_full(solution_buckets):
        ...
        while True:
            ...
            if goal > 0:
                ...
                if sol_cost < best_cost:
                    ...
                    key = get_bucket_key(best_cost)
                    if key and len(solution_buckets[key]) < TARGET_PER_BUCKET:
                        traj = rollout_full_trajectory(path_nodes[0], actions, sst_params, sim_params, callables)
                        solution_buckets[key].append(traj)
                        print(f"Recorded {key}: Cost {best_cost:.4f}")
...
all_trajectories_flat = []
for key in sorted(solution_buckets.keys()):
    all_trajectories_flat.extend(solution_buckets[key])

# Save all trajectories into a single .npz file
# This will save them as 'arr_0', 'arr_1', etc.
np.savez_compressed("videos/trajectories_data.npz", *all_trajectories_flat)
print(f"Saved {len(all_trajectories_flat)} trajectories to trajectories_data.npz")
